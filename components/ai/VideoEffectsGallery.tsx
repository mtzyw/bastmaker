...
function VideoEffectCard({ effect }: { effect: VideoEffectsGalleryItem }) {
      ...
      <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-white/10 bg-white/5 shadow-sm transition duration-300 group-hover:border-white/30 group-hover:shadow-lg group-hover:shadow-blue-500/20">
        {effect.previewVideoUrl ? (
          ...
          <video
            ref={videoRef}
            className="h-full w-full object-cover"
            src={FALLBACK_PREVIEW_VIDEO_URL}
            muted
            loop
            playsInline
            preload="metadata"
...
type VideoEffectsGalleryProps = {
  effects: VideoEffectsGalleryItem[];
};

export default function VideoEffectsGallery({ effects }: VideoEffectsGalleryProps) {
  return (
