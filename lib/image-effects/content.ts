...
const cache = new Map<string, ImageEffectCopy | null>();
...
async function readCopyFile(
  ...
  try {
    ...
    const parsed = JSON.parse(file) as ImageEffectCopy;
    ...
    if (
      ...
      console.error("[image-effects] failed to read localized copy", {
        locale,
        slug,
        error,
      });
...
export async function loadImageEffectCopy(
  slug: string,
  locale: Locale
): Promise<{ locale: Locale; copy: ImageEffectCopy | null }> {
  const normalizedLocale = (LOCALES.includes(locale)
    ? locale
...
export function clearImageEffectCopyCache() {
  cache.clear();
}
