...
const generatePresignedDownloadUrlSchema = z.object({
  key: z.string().min(1, "File key cannot be empty"),
});

type GeneratePresignedDownloadUrlInput = z.infer<
...
async function generatePresignedDownloadUrl(
  ...
  const validationResult = generatePresignedDownloadUrlSchema.safeParse(input);
  if (!validationResult.success) {
    const formattedErrors = validationResult.error.flatten().fieldErrors;
    return actionResponse.badRequest(
      `Invalid input: ${JSON.stringify(formattedErrors)}`
...
export async function generateAdminPresignedDownloadUrl(
  ...
  return generatePresignedDownloadUrl(input);
}
