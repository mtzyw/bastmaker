...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("[image-to-image-upload] failed to parse form data", error);
  ...
  const key = generateR2Key({
    fileName: `reference-${Date.now()}.${extension}`,
    path: "image-to-images",
  });
  ...
  try {
    ...
    console.error("[image-to-image-upload] failed to upload to R2", error);
