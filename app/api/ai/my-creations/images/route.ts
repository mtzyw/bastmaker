...
const IMAGE_MODALITY_CODES = ["t2i", "i2i"] as const;
...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await fetchUserCreations(supabase, user.id, page, pageSize, {
      modalityCodes: [...IMAGE_MODALITY_CODES],
    });
    ...
    console.error("[my-creations:image] failed to fetch items", error);
    return apiResponse.serverError("Failed to load image creations");
  }
}
