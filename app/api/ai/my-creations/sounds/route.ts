...
const SOUND_MODALITY_CODES = ["t2a"] as const;
...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await fetchUserCreations(supabase, user.id, page, pageSize, {
      modalityCodes: [...SOUND_MODALITY_CODES],
    });
    ...
    console.error("[my-creations:sound] failed to fetch items", error);
    return apiResponse.serverError("Failed to load sound creations");
  }
}
