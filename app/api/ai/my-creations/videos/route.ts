...
const VIDEO_MODALITY_CODES = ["t2v", "i2v"] as const;
...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await fetchUserCreations(supabase, user.id, page, pageSize, {
      modalityCodes: [...VIDEO_MODALITY_CODES],
    });
    ...
    console.error("[my-creations:video] failed to fetch items", error);
    return apiResponse.serverError("Failed to load video creations");
  }
}
