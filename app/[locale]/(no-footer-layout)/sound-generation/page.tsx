...
import SoundGenerationLeftPanel from "@/components/sound-generation/SoundGenerationLeftPanel";
import SoundGenerationRecentTasks from "@/components/sound-generation/SoundGenerationRecentTasks";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "SoundGenerationPage" });

  return constructMetadata({
    page: "SoundGeneration",
    title: t("metaTitle"),
...
export default async function SoundGenerationPage() {
  const t = await getTranslations("SoundGenerationPage");
  ...
  const rawHighlights = t.raw("sections.seo.highlights");
  const seoHighlights = Array.isArray(rawHighlights) ? (rawHighlights as string[]) : [];
  const sections: [SectionConfig, SectionConfig, SectionConfig, SectionConfig] = BASE_SECTIONS.map(
    (section, index) => {
      if (index === 2) return { ...section, title: t("sections.overview.title") };
      if (index === 3) return { ...section, title: t("sections.seo.title") };
      return section;
    },
  ...
  return (
    <PureFourSections
      ...
      sections={sections}
      withSidebar={false}
      section2Left={<SoundGenerationLeftPanel />}
      section2Right={<SoundGenerationRecentTasks hideEffectBadge />}
      mergedSectionContent={
        <SoundGenerationSeoContent
...
type SoundGenerationSeoContentProps = {
  badgeLabel: string;
...
function SoundGenerationSeoContent({
  badgeLabel,
  overviewTitle,
  overviewBody,
  overviewPoints,
  ...
  seoBody,
  seoHighlights,
}: SoundGenerationSeoContentProps) {
  return (
    <div className="container mx-auto px-4 md:px-8 py-16 space-y-14">
      <section className="space-y-6">
        <div className="max-w-4xl space-y-3">
          ...
          <h2 className="text-3xl font-semibold text-white">{overviewTitle}</h2>
          <p className="text-base leading-relaxed text-white/80">{overviewBody}</p>
        </div>
