--- [UPDATE] grant_subscription_credits_and_log
...
--- [ADD] initialize_or_reset_yearly_allocation
...
--- [ADD] allocate_specific_monthly_credit_for_year_plan
...
--- [UPDATE] grant_welcome_credits_and_log
DROP FUNCTION IF EXISTS public.grant_welcome_credits_and_log(uuid, integer);
CREATE OR REPLACE FUNCTION public.grant_welcome_credits_and_log(
    p_user_id uuid,
    p_welcome_credits integer DEFAULT 30
)
RETURNS TABLE(one_time_credits_balance INT, subscription_credits_balance INT, balance_jsonb JSONB) AS $$
DECLARE
    v_new_one_time_balance integer;
    v_new_subscription_balance integer;
